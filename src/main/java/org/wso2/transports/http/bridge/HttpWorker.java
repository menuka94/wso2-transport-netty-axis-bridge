...
public class HttpWorker implements Runnable {
    ...
    public void run() {
        ...
        processEntityEnclosingRequest(msgCtx);
    }

    private void processHttpRequestUri(MessageContext msgCtx) {

        msgCtx.setProperty(Constants.Configuration.HTTP_METHOD,
                incomingCarbonMsg.getProperty(BridgeConstants.HTTP_METHOD));
            ...
            if (localAddr != null) {
                servicePrefix =
                        incomingCarbonMsg.getProperty(org.wso2.transport.http.netty.contract.Constants.PROTOCOL) +
                                servicePrefixIndex + localAddr.getHostName() + ":" +
    ...
    public void processEntityEnclosingRequest(MessageContext msgCtx) {
        try {
            ...
            String method = (String) incomingCarbonMsg.getProperty(BridgeConstants.HTTP_METHOD);
            ...
            msgCtx.setProperty(HTTPConstants.HTTP_METHOD, method);
            ...
            int soapVersion =
                    RequestUtils.initializeMessageContext(msgCtx, soapAction, requestUri, contentTypeHeader);
